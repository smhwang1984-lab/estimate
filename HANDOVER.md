...
정리한 것이다. 마지막 갱신은 **v0.1.4 (2026-08-10)** 기준이다.
...
- 서버·통신 기능은 **없다**. v0.1.4부터 데이터 폴더를 **네트워크 공유 폴더**로 지정할 수
  있지만, 그건 평범한 파일 입출력(SMB)일 뿐 HTTP·TLS를 쓰지 않는다
  (업데이트도 여전히 `{설치폴더}\update`에 설치 파일을 두면 감지하는 방식이다)
...
|---|---|
| 버전 | v0.1.4 |
| 브랜치 | `worktree-v0.0.9` — v0.0.9~v0.1.4가 함께 커밋돼 있다 — **`Estimate` 브랜치에 미병합** |
| 원격 | **없음** (로컬 전용 저장소) |
| 주 개발 브랜치 | `Estimate` (기본 브랜치는 `main`이지만 실제 작업은 `Estimate`에서 해 왔다) |
| 설치 파일 | `installer\Output\MachineEstimate_Setup_v0.1.4.exe`. 워크트리 밖 배포 폴더 `C:\Users\SumH\orca\workspaces\Estimate\Estimate\installer\Output\`에도 복사해 뒀다 |
...
**병합은 아직 하지 않았다.** 사용자가 v0.1.4를 실사용으로 확인한 뒤 `Estimate` 브랜치로
합치는 것이 남은 절차다 — v0.0.9~v0.1.3도 아직 병합 전이라 이번에 같이 합치게 된다.
...
estimate_app/
  ...
  core/                     화면과 무관한 로직
    ...
    datastore.py            ★ v0.1.4 신설. 데이터를 어느 폴더에 둘지(로컬/네트워크 공유) + 원자적 쓰기
    library.py              ★ v0.1.4 신설. 견적 보관함(카드 목록을 이름 붙여 저장/불러오기)
    config.py               theme.json·rates.json 읽기, 공정 키 정의(MACHINE_KEYS)
    settings.py             ★ v0.0.9 신설. 설정창 값의 저장소(v0.1.4부터 공유 폴더에 놓일 수 있다)
  ...
  ui/
    ...
    popup.py                카드 입력창
    settings_dialog.py      ★ v0.0.9 신설. 설정창(v0.1.1 "가공 조건", v0.1.4 "데이터 위치" 탭 포함)
    library_dialog.py       ★ v0.1.4 신설. 견적 보관함 창(저장/불러오기/삭제)
...
| 14 | `session_state.json` / `update_state.txt`를 데이터 폴더(공유)로 옮기기 | 세션은 **자동 저장**이라 두 사람이 같이 켜 두면 나중에 끈 쪽이 앞사람 작업을 말없이 덮어쓴다(자동이라 물어볼 자리가 없다). update_state는 "이 PC가 이미 처리한 설치 파일" 기록이라 공유하면 B PC가 업데이트 안내를 영영 못 받는다. 둘 다 `paths.get_user_dir()`(로컬)에 그대로 둘 것 — 공유는 `datastore.get_data_dir()`이다 |
| 15 | 공유 폴더 시대에 메모리에 든 설정을 통째로 저장하기 | `app.settings`는 켤 때 읽은 사본이다. 그 사이 다른 PC가 단가를 고쳤으면 통째 저장은 그 수정을 날린다. 열 폭 저장(`table._persist_column_widths`)은 v0.1.4에서 **파일을 다시 읽어 열 폭만 얹는** 방식으로 바꿨다. 설정창의 [저장]은 여전히 통째 저장이다(사용자가 명시적으로 누른 것이라 남겨 뒀다 — 9절 참고) |
| 16 | 설정을 못 읽은 상태에서 저장하기 | 네트워크가 끊기면 `settings.load()`가 기본값을 돌려준다. 그 상태로 저장하면 공유 settings.json이 **모두에게** 기본값이 된다. `settings.save()`가 `_load_error`가 서 있으면 거부하도록 막아 뒀다 — 이 방어를 빼지 말 것 |
...
```
%LOCALAPPDATA%\MachineEstimate\     ← 항상 로컬. paths.get_user_dir()
  data_location.json   ★ v0.1.4. 공유 폴더가 어디인지 적어 둔 쪽지. 반드시 로컬이다
                       (공유 위치를 가리키는 쪽지를 공유 위치에 둘 수는 없다)
  session_state.json   작업 중 카드 목록 (자동 저장 — 공유하지 않는다, 5절 함정 14)
  update_state.txt     이미 처리한 업데이트 파일 기록 (PC별)
  settings.json        데이터 폴더를 따로 지정하지 않았을 때의 설정 파일
```

v0.1.4부터 아래 둘은 **데이터 폴더**(`datastore.get_data_dir()`)에 놓인다. 기본값은 위의
로컬 폴더와 같은 자리이고, [설정 > 데이터 위치]에서 네트워크 공유 폴더로 바꿀 수 있다.

```
{데이터 폴더}\
  settings.json        단가·Material·소재 비중·가공 조건·회사/견적서 양식
  estimates\*.json     견적 보관함 (파일 하나가 견적 한 건, core/library.py)
```

`theme.json` / `rates.json`을 **로컬** 폴더에 같은 이름으로 두면 번들 것보다 **우선**
적용된다(`paths.get_asset_path`). 색이나 열 폭 실험에 유용하다 — 이건 데이터 폴더가 아니라
로컬 폴더를 본다(PC마다 다른 값이어도 되는 화면 실험용이기 때문).
...
- [ ] **v0.1.4(와 아직 안 합친 v0.0.9~v0.1.3) 실사용 확인 후 `Estimate` 브랜치로 병합.**
      현재 `worktree-v0.0.9`에만 있다.
- [ ] **진짜 네트워크 공유 폴더에서 확인.** v0.1.4의 검증은 전부 로컬 폴더를 공유 폴더인 척
      쓴 것이다. 실제 SMB에서 `os.replace`의 원자성, 공유 폴더에 임시 파일 만들기,
      연결이 끊겼을 때의 대기 시간(`datastore.PROBE_TIMEOUT` 4초로 잘리는지)은 못 봤다.
- [ ] **두 PC 동시 사용 확인.** 같은 견적을 양쪽에서 열어 두고 저장했을 때 "다른 사람이
      고쳤습니다" 경고가 실제로 뜨는지는 파일 mtime 비교로만 확인했다.
...
- **설정창 [저장]은 여전히 통째 저장이다.** 공유 폴더를 쓰는 중에 A가 설정창을 열어 둔 채로
  B가 단가를 고치면, A가 저장하는 순간 B의 수정이 사라진다. 사용자가 직접 [저장]을 누른
  경우라 v0.1.4에서는 그대로 뒀다(열 폭 저장은 UI 제스처라 고쳤다 — 5절 함정 15).
  고치려면 저장 직전에 파일을 다시 읽어 바뀐 항목만 비교해 알리는 방식이 필요하다.
- **공유 폴더 상태는 프로그램을 켤 때 한 번만 판정한다**(`datastore._state` 캐시).
  실행 중에 네트워크가 끊기거나 다시 붙어도 자동으로 알아채지 못한다. [설정 > 데이터 위치 >
  다시 확인]을 누르거나 프로그램을 다시 켜야 한다.
- **보관함에는 잠금이 없다.** 일부러 그렇게 했다(프로그램이 비정상 종료하면 잠금 파일이
  남아 남의 작업을 막아 놓고 풀지 못하는 쪽이 더 큰 사고다). 대신 저장 직전 mtime 비교로
  "다른 사람이 고쳤습니다"를 알린다.
...
| **v0.1.4** | **데이터 저장 위치 선택(네트워크 공유)** — [설정 > 데이터 위치]에서 폴더를 고르면 `settings.json`(단가·소재·비중·가공조건·회사)과 **견적 보관함**(`estimates\*.json`)이 그 폴더로 간다. 세션 자동 저장·업데이트 기록은 PC별 로컬 유지. **견적 보관함** 신설 — 상단 `견적 불러오기` / `견적 저장`, 이름 붙여 저장하고 목록에서 골라 불러온다(불러오면 카드 목록을 통째로 대체, 저장 시 mtime 비교로 "다른 사람이 고쳤습니다" 경고). 안전장치: 설정 읽기 실패 시 저장 거부, 임시 파일+`os.replace` 원자적 쓰기, 끊긴 경로 4초 제한 |
